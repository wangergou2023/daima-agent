...
static int work_item_tool_probe(struct device *dev)
{
    (void)dev;
    return 0;
}

static struct tool_device s_work_item_device = {
    .name = "work_item",
    .description = "收集和管理结构化 work item。用于记录 bug、功能缺失、改进、技术债、文档缺口和测试缺口。",
    .input_schema_json = "{\"type\":\"object\"," "\"properties\":{" "\"action\":{\"type\":\"string\",\"description\":\"add / list / update / review / summary，默认 list\"}," "\"id\":{\"type\":\"string\",\"description\":\"update 时使用的 work item ID\"}," "\"type\":{\"type\":\"string\",\"description\":\"defect / missing / improvement / tech_debt / docs / test_gap\"}," "\"source\":{\"type\":\"string\",\"description\":\"user / log / test / github_issue / heartbeat / review\"}," "\"title\":{\"type\":\"string\",\"description\":\"事项标题，add 时必填\"}," "\"description\":{\"type\":\"string\",\"description\":\"背景、上下文和影响\"}," "\"expected\":{\"type\":\"string\",\"description\":\"期望结果\"}," "\"actual\":{\"type\":\"string\",\"description\":\"当前现象或缺口\"}," "\"evidence\":{\"type\":\"object\",\"description\":\"{session_id, issue_url, logs, files, commands}\"}," "\"status\":{\"type\":\"string\",\"description\":\"new / triaged / needs_info / accepted / planned / fixing / done / rejected\"}," "\"priority\":{\"type\":\"string\",\"description\":\"P0 / P1 / P2 / P3\"}," "\"limit\":{\"type\":\"integer\",\"description\":\"list/summary 最多展示条数，默认 20\"}," "\"ids\":{\"type\":\"array\",\"items\":{\"type\":\"string\"},\"description\":\"review 批量操作时使用的 work item ID 数组\"}" "}," "\"required\":[]}",
};
