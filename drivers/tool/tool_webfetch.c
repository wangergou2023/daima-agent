...
static int webfetch_tool_probe(struct device *dev)
{
    (void)dev;
    return 0;
}

static struct tool_device s_webfetch_device = {
    .name = "webfetch",
    .description = "从指定 URL 获取网页内容，支持返回纯文本或原始 HTML。用于搜索信息、阅读文档、查阅 API 参考等。",
    .input_schema_json = "{\"type\":\"object\"," "\"properties\":{" "\"url\":{\"type\":\"string\",\"description\":\"要获取的完整 URL（必须 http:// 或 https://）\"}," "\"format\":{\"type\":\"string\",\"description\":\"返回格式：text（纯文本，默认）或 html（原始 HTML）\"}" "}," "\"required\":[\"url\"]}",
};
