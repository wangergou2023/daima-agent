...
static int log_tool_probe(struct device *dev)
{
    (void)dev;
    return 0;
}

static struct tool_device s_log_device = {
    .name = "log_tool",
    .description = "读取 agent 自身运行日志，用于诊断工具失败、网络错误、系统异常。支持 tail / search / errors。",
    .input_schema_json = "{\"type\":\"object\"," "\"properties\":{" "\"action\":{\"type\":\"string\",\"description\":\"tail / search / errors，默认 tail\"}," "\"pattern\":{\"type\":\"string\",\"description\":\"search 时使用的搜索关键词（大小写不敏感）\"}," "\"lines\":{\"type\":\"integer\",\"description\":\"输出最近多少行，默认 50，最大 200\"}" "}," "\"required\":[]}",
};
