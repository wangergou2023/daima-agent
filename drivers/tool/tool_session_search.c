...
static int session_search_tool_probe(struct device *dev)
{
    (void)dev;
    return 0;
}

static struct tool_device s_session_search_device = {
    .name = "session_search",
    .description = "搜索历史会话消息和事实卡片，也可列出已有会话。适合回忆之前聊过什么、查看上下文压缩摘要或事实卡片。",
    .input_schema_json = "{\"type\":\"object\"," "\"properties\":{" "\"query\":{\"type\":\"string\",\"description\":\"可选搜索词；不传时返回会话概览\"}," "\"chat_id\":{\"type\":\"string\",\"description\":\"可选：只搜索某个会话，例如 web_1dfazy\"}," "\"target\":{\"type\":\"string\",\"description\":\"messages / facts / summaries / both，默认 both\"}," "\"output_mode\":{\"type\":\"string\",\"description\":\"hits / sessions；有 query 默认 hits，无 query 默认 sessions\"}," "\"limit\":{\"type\":\"integer\",\"description\":\"返回条数上限（可选）\"}," "\"offset\":{\"type\":\"integer\",\"description\":\"分页偏移（可选）\"}" "}," "\"required\":[]}",
};
...
{
    return &s_session_search_device;
}

const struct tool_driver *tool_session_search_driver(void)
{
    return &s_session_search_driver;
}
