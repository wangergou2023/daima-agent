...
static int drive_straight_tool_probe(struct device *dev)
{
    (void)dev;
    return 0;
}

static struct tool_device s_drive_straight_device = {
    .name = "robot_drive_straight",
    .description = "让 Vector 机器人直行前进或后退。speed_mmps: 速度 mm/s (正向前进，负向后腿，推荐 50-200)。dist_mm: 距离 mm。",
    .input_schema_json = "{\"type\":\"object\"," "\"properties\":{" "\"speed_mmps\":{\"type\":\"number\",\"description\":\"速度 mm/s (正=前进，负=后退)\"}," "\"dist_mm\":{\"type\":\"number\",\"description\":\"距离 mm\"}" "},\"required\":[\"speed_mmps\",\"dist_mm\"]}",
};
...
static int turn_in_place_tool_probe(struct device *dev)
{
    (void)dev;
    return 0;
}

static struct tool_device s_turn_in_place_device = {
    .name = "robot_turn_in_place",
    .description = "让 Vector 机器人原地转向。angle_rad: 角度(弧度), pi/2=右转90°。",
    .input_schema_json = "{\"type\":\"object\"," "\"properties\":{" "\"angle_rad\":{\"type\":\"number\",\"description\":\"转向角度(弧度)，正值右转\"}" "},\"required\":[\"angle_rad\"]}",
};
...
static int drive_wheels_tool_probe(struct device *dev)
{
    (void)dev;
    return 0;
}

static struct tool_device s_drive_wheels_device = {
    .name = "robot_drive_wheels",
    .description = "控制 Vector 左右轮速度实现曲线或原地旋转。left_mmps: 左轮速度 mm/s, right_mmps: 右轮速度 mm/s。",
    .input_schema_json = "{\"type\":\"object\"," "\"properties\":{" "\"left_mmps\":{\"type\":\"number\",\"description\":\"左轮速度 mm/s\"}," "\"right_mmps\":{\"type\":\"number\",\"description\":\"右轮速度 mm/s\"}" "},\"required\":[\"left_mmps\",\"right_mmps\"]}",
};
