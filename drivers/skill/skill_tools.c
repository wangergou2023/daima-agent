...
static err_t activate_loaded_bundle(skill_tool_bundle_slot_t *slot)
{
    ...
    for (int i = 0; i < slot->bundle.tool_count; i++) {
        err_t err = tool_dynamic_bus_register(&slot->bundle.tools[i]);
        if (err != 0) {
            for (int j = 0; j < i; j++) {
                tool_dynamic_bus_unregister(slot->bundle.tools[j].name);
            }
            return err;
        }
    }
...
static err_t init_tool_from_json(skill_tool_bundle_slot_t *slot, cJSON *item)
{
    ...
    err_t err = tool_dynamic_bus_register(&tool);
    if (err != 0) {
        return err;
    }
...
err_t skill_tools_unregister(const char *skill_name)
{
    ...
    for (int i = 0; i < slot->bundle.tool_count; i++) {
        tool_dynamic_bus_unregister(slot->bundle.tools[i].name);
    }
