...
int main()
{
	...
	/* step.1 通过系统函数获取当前时间 */
	/* step.1 Get the current time through system functions */
	SUTime now_tm;
	...
	/* step.5 创建线程，等待alarm时间到达 */
	/* step.5 Create thread for wait alarm time finsh */
	pthread_t alarm_tid;
	...
	/* step.7 通过rtc获取当前时间 */
 	/* step.7 Through rtc to Get now time */
	ret = SU_Base_GetTime(&now_tm);
	if (ret < 0) {
		printf("SU_Base_GetTime() error\n");
		return ret;
