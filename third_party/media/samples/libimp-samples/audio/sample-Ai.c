...
static void *_ai_basic_record_test_thread(void *argv)
{
	...
	while(1) {
		...
		/* Step.10 将记录数据保存到文件 */
		/* Step.10 Save the recording data to a file */
		fwrite(frm.virAddr, 1, frm.len, record_file);
		/* Step.11 释放录音帧 */
		/* Step.11  release the audio record frame */
	...
	/* Step.13 禁用音频通道 */
	/* Step.13 disable the audio channel */
	...
	/* Step.14 禁用音频设备 */
	/* Step.14 disable the audio devices */
