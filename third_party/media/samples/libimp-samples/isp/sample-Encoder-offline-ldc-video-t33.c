...
#define TAG "sample-Encoder-offline-ldc-video-t33"

/* 0:close  1:LDC 2:I2D */
#define LDC_ONLINE_MODE  2

extern struct chn_conf chn[];
extern int direct_switch;
extern int g_t33_offline_ldc_mode;
...
int main(int argc, char *argv[])
{
	...
	/* Step.1 系统初始化 */
	/* Step.1 System init */
	g_t33_offline_ldc_mode = LDC_ONLINE_MODE;
	...
	if (g_t33_offline_ldc_mode == 1) {
		IMPFSChnLdcAttr ldc_attr;
		int chn = 0;
		char lut_name[128] = "/etc/sensor/ldc_lut_1920_1080_8_t33.bin";

		printf("Using T33 Offline LDC mode!\n");
		ldc_attr.enable = IMPFS_LDC_ENABLE;
		ldc_attr.size   = IMPFS_LUT_BLOCKSIZE_8x8;
		memcpy(&ldc_attr.lut_name, lut_name, sizeof(char)*128);
		ret = IMP_FrameSource_SetChnAttr_Expand(chn, &ldc_attr);
	} else if (g_t33_offline_ldc_mode == 2) {
		printf("Using T33 Offline I2D mode!\n");

		chn[0].fs_chn_attr.i2dattr.i2d_enable = 1;
		chn[0].fs_chn_attr.i2dattr.rotate_enable = 1;
		chn[0].fs_chn_attr.i2dattr.rotate_angle = 180;
	}
