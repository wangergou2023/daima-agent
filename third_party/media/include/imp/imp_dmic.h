...
typedef enum {
	...
	DMIC_BIT_WIDTH_24 = 24, /**<24 bit 采样精度*/
} IMPDmicBitWidth;
...
/**
 * @fn int IMP_DMIC_EnableNs(IMPDmicAttr *attr, int mode);
 *
 * 启用指定音频输入的噪声抑制功能.
...
int IMP_DMIC_EnableNs(IMPDmicAttr *attr, int mode);

/**
 * @fn int IMP_DMIC_DisableNs(void)
 ...
 * @attention 无.
 */
int IMP_DMIC_DisableNs(void);

/**
 * @fn int IMP_DMIC_EnableAgc(IMPDmicAttr *attr, IMPAudioAgcConfig agcConfig)
 ...
 * @remarks 需要注意的是,AGC可以将声音的增益放大,但是如果增益的参数不合适,就会导致破音等情况,请在具体使用时>自行调整.

 * @attention 回声消除包含了 AGC 功能,如果启用回声抵消,就不需要做自动增益.
 */
int IMP_DMIC_EnableAgc(IMPDmicAttr *attr, IMPAudioAgcConfig agcConfig);

/**
 * @fn int IMP_DMIC_DisableAgc(void)
 ...
 * @attention 无.
 */
int IMP_DMIC_DisableAgc(void);

/**
 * @fn int IMP_DMIC_EnableHpf(IMPDmicAttr *attr)
 ...
 */
int IMP_DMIC_EnableHpf(IMPDmicAttr *attr);

/**
 * @fn int IMP_Dmic_SetHpfCoFrequency(int cofrequency)
 ...
 */
int IMP_DMIC_SetHpfCoFrequency(int cofrequency);
/**
 * @fn int IMP_DMIC_DisableHpf(void)
