...
#define SELECT_INTERNAL_MIC_EXTERNAL_SPK  2
#define SELECT_INTERNAL_SPK_EXTERNAL_MIC  3
 ...
 * snprintf(my_encoder.name, sizeof(my_encoder.name), "%s", "MY_G711A");
 ...
 * snprintf(my_decoder.name, sizeof(my_decoder.name), "%s", "MY_G711A");
...
/**
 * @fn int IMP_AI_SetAlgoOrder(int audioDevId, int aiChn ,AudioAI_Type order[])

 * 调整音频录音AI算法NS/AGC/HPF先后顺序.
 * @param[in] audioDevId 音频设备号.
 * @param[in] aiChn 音频输入通道号.
 * @param[in] order[] 算法先后顺序
 ...
 * int devID = 1;
 * int chnID = 0;
 * AudioAI_Type hw_order[] = {AI_MODULE_HPF, AI_MODULE_NS, AI_MODULE_AGC};
 * ret = IMP_AI_SetAlgoOrder(devID, chnID, hw_order);
 * if(ret != 0) {
 *		IMP_LOG_ERR(TAG, "Set Audio Algo order err: %d\n", ret);
 *		return ret;
 * }
 * @endcode
 ...
 * @attentio 无.
 */
int IMP_AI_SetAlgoOrder(int audioDevId, int aiChn ,AudioAI_Type order[]);

/**
 * @fn int IMP_AI_GetAlgoOrder(int audioDevId, int aiChn, AudioAI_Type order[]);

 * 获取音频录音AI算法NS/AGC/HPF先后顺序.
 * @param[in] audioDevId 音频设备号.
 * @param[in] aiChn 音频输入通道号.
 * @param[out] 当前算法处理顺序
 ...
 * @remarks 示例代码
 * @code
 * AudioAI_Type hw_order[AI_MODULE_BUTT];
 * ret = IMP_AI_GetAlgoOrder(devID, chnID, hw_order);
 * if(ret != 0) {
 *		IMP_LOG_ERR(TAG, "Get Audio Algo order err: %d\n", ret);
 *		return ret;
 * }else{
 *		for (int i = 0; i < AI_MODULE_BUTT; i++) {
 *			IMP_LOG_INFO(TAG,"AI Algo Module %d: %d\n",i,hw_order[i]);
 *		}
 * }
 * @endcode
 ...
 * @attentio 无.
 */
int IMP_AI_GetAlgoOrder(int audioDevId, int aiChn, AudioAI_Type order[]);

/**
 * @fn int IMP_AO_SetAlgoOrder(int audioDevId, int aoChn ,AudioAO_Type order[])

 * 调整音频放音AO算法AGC/HPF先后顺序.
 * @param[in] audioDevId 音频设备号.
 * @param[in] aoChn 音频输出通道号.
 * @param[in] order[] 算法先后顺序
 ...
 * int devID = 0;
 * int chnID = 0;
 * AudioAO_Type hw_order[] = {AO_MODULE_AGC, AO_MODULE_HPF};
 * ret = IMP_AO_SetAlgoOrder(devID, chnID, hw_order);
 * if(ret != 0) {
 *		IMP_LOG_ERR(TAG, "Set Audio Algo order err: %d\n", ret);
 *		return ret;
 * }
 ...
 * @attentio 无.
 */
int IMP_AO_SetAlgoOrder(int audioDevId, int aoChn ,AudioAO_Type order[]);

/**
 * @fn int IMP_AO_GetAlgoOrder(int audioDevId, int aoChn, AudioAO_Type order[]);

 * 获取音频放音AO算法AGC/HPF先后顺序.
 * @param[in] audioDevId 音频设备号.
 * @param[in] aoChn 音频输入通道号.
 * @param[out] 当前算法处理顺序
 ...
 * @remarks 示例代码
 * @code
 * AudioAO_Type hw_order[AO_MODULE_BUTT];
 * ret = IMP_AO_GetAlgoOrder(devID, chnID, hw_order);
 * if(ret != 0) {
 *		IMP_LOG_ERR(TAG, "Get Audio Algo order err: %d\n", ret);
 *		return ret;
 * }else{
 *		for (int i = 0; i < AO_MODULE_BUTT; i++) {
 *			IMP_LOG_INFO(TAG,"AO Algo Module %d: %d\n",i,hw_order[i]);
 *		}
 * }
 * @endcode
 ...
 * @attentio 无.
 */
int IMP_AO_GetAlgoOrder(int audioDevId, int aoChn, AudioAO_Type order[]);
 ...
 * @param[in] audioDevId 音频设备号.
 * @param[in] aiChn 音频输入通道号.
 * @param[in] codec 音频内外部codec参数.
	SELECT_EXTERNAL：外部codec
	SELECT_INTERNAL：内部codec
	SELECT_INTERNAL_MIC_EXTERNAL_SPK：内部codec录音外部codec放音
	SELECT_INTERNAL_SPK_EXTERNAL_MIC：内部codec放音外部codec录音
