...
enum
{
	IMP_OK_OSD_ALL					= 0x0 ,			/* 运行正常 */
	/* OSD */
	IMP_ERR_OSD_CHNID				= 0x80020001,	/* 通道 ID 超出合法范围 */
	IMP_ERR_OSD_PARAM				= 0x80020002,	/* 参数超出合法范围 */
	IMP_ERR_OSD_EXIST				= 0x80020004,	/* 试图申请或者创建已经存在的设备、通道或者资源 */
	IMP_ERR_OSD_UNEXIST				= 0x80020008,	/* 试图使用或者销毁不存在的设备、通道或者资源 */
	IMP_ERR_OSD_NULL_PTR			= 0x80020010,	/* 函数参数中有空指针 */
	IMP_ERR_OSD_NOT_CONFIG			= 0x80020020,	/* 使用前未配置 */
	IMP_ERR_OSD_NOT_SUPPORT			= 0x80020040,	/* 不支持的参数或者功能 */
	IMP_ERR_OSD_PERM				= 0x80020080,	/* 操作不允许 */
	IMP_ERR_OSD_NOMEM				= 0x80020100,	/* 分配内存失败 */
	IMP_ERR_OSD_NOBUF				= 0x80020200,	/* 分配缓冲区失败 */
	IMP_ERR_OSD_BUF_EMPTY			= 0x80020400,	/* 缓冲区中无数据 */
	IMP_ERR_OSD_BUF_FULL			= 0x80020800,	/* 缓冲区中数据满 */
	IMP_ERR_OSD_BUF_SIZE			= 0x80021000,	/* 缓冲区空间不足 */
	IMP_ERR_OSD_SYS_NOTREADY		= 0x80022000,	/* 系统没有初始化或没有加载相应模块 */
	IMP_ERR_OSD_OVERTIME			= 0x80024000,	/* 等待超时 */
	IMP_ERR_OSD_RESOURCE_REQUEST	= 0x80028000,	/* 资源创建、申请失败 */
};
...
typedef struct {
	IMPISPDrawBlockAttr stDrawAttr;		 /*ISP绘制线、框属性*/
	IMPISPOSDBlockAttr	stpicAttr;		 /*ISP绘制图片属性*/
	IMPISPMaskBlockAttr stCoverAttr;	 /*ISP绘制矩形遮挡*/
}IMPOSDIspDraw;
...
typedef struct mosaicPointAttr{
	int x;					/* 马赛克起始x坐标 (2对齐)*/
	int y;					/* 马赛克起始y坐标 (2对齐)*/
	int mosaic_width;		/* 所需马赛克宽度 (2对齐且加上x坐标不超过通道总宽)*/
	int mosaic_height;		/* 所需马赛克高度 (2对齐且加上y坐标不超过通道总高)*/
	int frame_width;		/* 通道分辨率宽度 */
	int frame_height;		/* 通道分辨率高度 */
	int mosaic_min_size;	/* 最小马赛克单位 (2对齐)*/
} IMPOSDMosaicAttr;

/**
 * 2bit属性
 */
typedef struct twoBitAttr{
	uint32_t twobit_mask;			/* 2bit 标识位 */
	uint32_t twobit_bit0_fmt;		/* bit 0 参数 高8位表示透明度，其他位表示颜色 */
	uint32_t twobit_bit1_fmt;		/* bit 1 参数 高8位表示透明度，其他位表示颜色 */
} IMPOSD2BitAttr;
 ...
 *
 * 配置OSD update回调函数,可以用于获取OSD Frame
 *
 * @param[in] grpNum   Group号
 ...
 * @remark 无
 *
 * @attention 在IMP_OSD_CreateGroup 之后且在IMP_FrameSource_EnableChn之前调用.
 */
