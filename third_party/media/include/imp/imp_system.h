 ...
 *
 * @param[in] poolId:  释放mempool rmem 内存
 *
 * @retval:  等于0, 成功
 * @retval:  小于0, 失败
 *
 * @attention: 内存池释放, 在码流运行时, 禁止释放,只有在所有资源退出后, 才能够进行释放
 * 且由于sdk中, 相关内存申请并不走内存池, 若频繁申请释放, 可能由于内存碎片导致申请
 * 失败的情况, 一般建议 只有在需要rmem 内存用作他用时, 进行内存池释放
 */
int IMP_System_MemPoolFree(int poolId);

/**
 * @brief IMP_Buf_CreatePool
 * @param 创建用户端使用的rmem内存池
 * @param[in/out] poolID: 输出参数，创建成功后存放缓冲区池的ID，该poolID用于IMP_Buf_Alloc
 * @param[in] size: 缓冲区池的总内存大小（字节）
 * @param[in] name: 缓冲区池的名称
 * @param[in] alignsize: 缓冲区池的内存对齐大小（字节，通常为2的幂次）
 *
 * @retval:  成功，返回0
 * @retval:  失败，返回非0错误码
 *
 * @attention: 创建的缓冲区池需通过IMP_Buf_DestroyPool主动销毁，否则会造成内存泄漏；alignsize需为合法的对齐值。
 *
 */
int32_t IMP_Buf_CreatePool(uint32_t *poolID, uint32_t size, const char *name, uint32_t alignsize);

/**
 * @brief IMP_Buf_DestroyPool
 * @param 销毁用户端使用的rmem内存池
 * @param[in] poolID: 要销毁的缓冲区池ID（IMP_Buf_CreatePool创建的poolID）
 * @param[in] name: 缓冲区池的名称（用于校验，确保销毁正确的池）
 *
 * @retval:  成功，返回0
 * @retval:  失败，返回非0错误码
 *
 * @attention: 销毁前需确保池内所有已分配的缓冲区均已通过IMP_Buf_Free释放，否则会导致内存泄漏或程序异常。
 *
 */
int32_t IMP_Buf_DestroyPool(uint32_t poolID, const char *name);

/**
 * @brief IMP_Buf_Alloc
 * @param 申请IMP_Buf_CreatePool的内存池内存
 * @param[in] poolID: 要分配内存的缓冲区池ID（IMP_Buf_CreatePool创建的poolID）
 * @param[in] poolname: 缓冲区池的名称（用于校验）
 * @param[in] size: 要分配的内存块大小（字节）
 * @param[in] name: 分配的内存块名称（用于标识和调试）
 * @param[in] alignsize: 内存块的对齐大小（字节）
 *
 * @retval:  成功，返回分配的内存地址
 * @retval:  失败，返回NULL
 *
 * @attention: 需确保poolID有效且池内剩余空间足够分配指定大小的内存；分配的内存必须通过IMP_Buf_Free释放。
 *
 */
void* IMP_Buf_Alloc(uint32_t poolID, const char *poolname, uint32_t size, const char *name, uint32_t alignsize);

/**
 * @brief IMP_Buf_Free
 * @param 释放IMP_Buf_CreatePool内存池内存
 * @param[in] poolID: 内存块所属的缓冲区池ID（IMP_Buf_CreatePool创建的poolID）
 * @param[in] addr: 要释放的内存地址（必须是从该池分配的有效地址）
 *
 * @retval:  成功，返回0
 * @retval:  失败，返回非0错误码
 *
 * @attention: 禁止重复释放同一地址、释放非该池分配的地址，否则会导致内存错误或程序崩溃。
 *
 */
int32_t IMP_Buf_Free(uint32_t poolID, void *addr);
