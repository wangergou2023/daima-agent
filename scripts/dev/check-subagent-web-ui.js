...
const SESSION_STATE_RUNTIME_JS_PATH = `${ROOT}/spiffs_data/web/session-state-runtime.js`;
...
function createFetchStub() {
  ...
  const fetchStub = async (url, options = {}) => {
    ...
    async json() {
          ...
          items: [
            {
              ...
              child_session: {
                ...
                cursor: {
                  history: { after_seq: 20, visible_seq: 21, first_visible_seq: 21, replay_reset: false },
                  frames: { after_seq: 3, visible_seq: 4, first_visible_seq: 4, replay_reset: false },
                  commits: { after_seq: 3, visible_seq: 4, first_visible_seq: 4, replay_reset: false },
                },
            ...
            },
            {
              ...
              child_session: {
                ...
                cursor: {
                  history: { after_seq: 18, visible_seq: 31, first_visible_seq: 31, replay_reset: true },
                  frames: { after_seq: 2, visible_seq: 31, first_visible_seq: 31, replay_reset: true },
                  commits: { after_seq: 2, visible_seq: 31, first_visible_seq: 31, replay_reset: true },
                },
                pending_queue: { permissions: [], questions: [] },
              },
            },
      ...
      if (text.includes('/api/session_events?chat_id=web_test')) {
        return {
          chat_id: 'web_test',
          stream_kind: 'session_events',
          cursor: {
            after_seq: 0,
            visible_seq: 20,
            first_visible_seq: 1,
          ...
          events: [
            {
              ...
              seq: 1,
              payload: { seq: 1, role: 'user', content: '帮我分析目录结构' },
            },
            {
              id: 'evt_hist_2',
              type: 'history_message',
              seq: 2,
              payload: { seq: 2, role: 'assistant', content: '正在恢复多 subagent 会话' },
            },
            {
              id: 'evt_subagent_bootstrap',
          ...
          history: [
            ...
            { seq: 2, role: 'assistant', content: '正在恢复多 subagent 会话' },
          ],
          subagent: snapshotData,
        };
      }
      if (text.includes('/api/session_events?chat_id=web_question')) {
        return {
          chat_id: 'web_question',
          stream_kind: 'session_events',
          cursor: {
...
function bootstrapApp(dom) {
  ...
  const sessionStateRuntimeJs = fs.readFileSync(SESSION_STATE_RUNTIME_JS_PATH, 'utf8');
  vm.runInContext(sessionStateRuntimeJs, context);
...
async function main() {
  ...
  expect(
    fetchStub.requests.some((url) =>
      String(url).includes(`/api/session_events?chat_id=${currentChatId}`) ||
  ...
  expect(
    ...
    'expected HTTP subagent bootstrap to request chat delta replay with coordinator revision and explicit child session cursors',
  );
  expect(
  ...
  const unifiedSessionTransport = transportFactory({
    async fetchImpl(url) {
      const text = String(url || '');
      if (text.includes('/api/session_events?chat_id=web_session_first')) {
              ...
              events: [
                {
                  ...
                  payload: {
                    ...
                    seq: 1,
                    role: 'assistant',
                    content: 'session-first history',
                    reasoning: 'session-first reasoning',
                  },
                },
                {
                  id: 'evt_session_first_coord_1',
                  seq: 2,
                  ...
                  payload: {
                    ...
                    agents: [
                      {
                        task_id: 'dt_session_first_app',
                        session_id: 'delegate_sync_session_first_app',
                        subagent_type: 'explore',
                        description: '分析 packages/app',
                        status: 'running',
                      },
                    ],
                  },
                ...
                {
                  id: 'evt_session_first_subagent_1',
                  type: 'subagent_session',
                  seq: 3,
                  payload: {
                    coordinator_id: 'dc_session_first',
                    ...
                    agent: {
                      ...
                      description: '分析 packages/app',
                      child_session: {
                        summary: 'session-events app summary',
                        history: [
                          {
                            id: 'session-first-child-history-1',
                            seq: 14,
                            role: 'assistant',
                            content: 'session-events child history',
                          },
                        ],
                        frames: [
                          {
                            id: 'session-first-child-frame-1',
                            seq: 8,
                            type: 'subagent_progress',
                            phase: 'progress',
                            status: 'running',
                            task: '分析 packages/app',
                            detail: 'session-events child frame',
                            output_preview: 'session-events app summary',
                            ts: 8,
                          },
                        ],
                        commits: [
                          {
                            id: 'session-first-child-commit-1',
                            seq: 8,
                            kind: 'progress',
                            phase: 'running',
                            status: 'running',
                            label: '分析 packages/app',
                            text: 'session-events child frame',
                            ts: 8,
                          },
                        ],
                        window: {
                          history_after_seq: 13,
                          frame_after_seq: 7,
                          commit_after_seq: 7,
                          history_last_seq: 14,
                          frame_last_seq: 8,
                        ...
                        cursor: {
                          history: { after_seq: 13, visible_seq: 14, first_visible_seq: 14, replay_reset: false },
                          frames: { after_seq: 7, visible_seq: 8, first_visible_seq: 8, replay_reset: false },
                          commits: { after_seq: 7, visible_seq: 8, first_visible_seq: 8, replay_reset: false },
                        },
                        pending_queue: { permissions: [], questions: [] },
                      },
                    },
      ...
      if (text.includes('/api/session_state?chat_id=web_session_first')) {
        fail(`unexpected legacy session_state fetch during unified session transport restore: ${text}`);
      }
      if (text.includes('/api/session_history')) {
    ...
    applySessionPayload(payload) {
      ...
      const key = String(session.task_id || session.session_id || '').trim();
      if (!key) return;
      unifiedReplayState.details.set(key, {
        task_id: key,
        session_id: String(session.session_id || '').trim(),
        child_session: agent.child_session || null,
      });
    },
    applyCoordinatorPayload(payload) {
      const coordinatorId = String(payload?.coordinator_id || '').trim();
      if (!coordinatorId) return;
      unifiedReplayState.coordinators.set(coordinatorId, payload);
      unifiedReplayState.liveCursor.visibleRevision = Math.max(
        Number(unifiedReplayState.liveCursor.visibleRevision) || 0,
        Number(payload?.visible_revision) || 0,
      );
    },
    getRuntimeState() {
  ...
  expect(sessionStateFetches.length === 1, 'expected unified transport restore to fetch /api/session_events once');
  expect(fallbackHistoryFetches === 0, 'expected unified transport restore to skip legacy session_history fetch');
  expect(fallbackSnapshotApplied === 1, 'expected unified transport restore to hydrate subagent snapshot from unified session_events response');
  expect(
    unifiedReplayState.details.has('dt_session_first_app'),
  ...
  expect(
    ...
    'expected unified transport restore result to expose session_events history and subagent restore flags',
  );

  const domSessionFirst = buildDom();
  const { fetchStub: sessionFirstFetchStub } = bootstrapApp(domSessionFirst);
  const baselineSessionEventsFetches = sessionFirstFetchStub.requests.filter((url) =>
