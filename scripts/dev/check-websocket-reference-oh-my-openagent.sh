...
REPO_ROOT="${OMO_ROOT:-$WORKSPACE_ROOT/oh-my-openagent}"
CLONE_URL="${OMO_CLONE_URL:-https://github.com/OpenAgentPlatform/oh-my-openagent.git}"
CHAT_ID="web_probe_omo_$(date +%s)"
RUN_LOG="/tmp/daima-agent-omo-run.log"
...
ensure_repo() {
  ...
  if [ -d "$REPO_ROOT" ] && [ ! -d "$REPO_ROOT/.git" ]; then
    echo "existing path is not a git repo: $REPO_ROOT" >&2
    return 1
  fi
...
python3 "$ROOT_DIR/scripts/dev/probe-subagent-websocket.py" \
  ...
  --port 1234 \
  --chat-id "$CHAT_ID" \
  --content "帮我分析 ${REPO_ROOT} 的目录结构和关键模块，要求同时安排多个 subagent，分别分析 packages、docs、assets，最后汇总。" \
  --timeout 180 \
